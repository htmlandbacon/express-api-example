...
describe('/api/users', function() {
    ...
    it('should return list of users when called', function(done) {
        superagent.get(`http://localhost:${port}/api/users`).end(function(err, res) {
            assert.ifError(err);
    ...
    describe('/:id', function() { 
        it('returns user when valid user id is used', function(done) {
                superagent.get(`http://localhost:${port}/api/users/${validUserID}`).end(function(err, res) {
                assert.ifError(err);
                assert.equal(res.status, status.OK);
        ...
        it('returns 404 error with message when user does not exist', function(done) {
                superagent.get(`http://localhost:${port}/api/users/${invalidUserID}`).end(function(err, res) {
